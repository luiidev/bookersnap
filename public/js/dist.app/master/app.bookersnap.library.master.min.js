...
return link;}};}]);}));var obtenerIdMicrositio=function(){var url=location.href;var pos=url.indexOf("ms");var id=url.substr(pos+3);var last_pos=id.indexOf("/");id=id.substr(0,last_pos);return id;};var getDaysWeek=function(){var days=[{id:0,label:'Domingo'},{id:1,label:'Lunes'},{id:2,label:'Martes'},{id:3,label:'Miercoles'},{id:4,label:'Jueves'},{id:5,label:'Viernes'},{id:6,label:'Sabado'},];return days;};var getGender=function(){var gender=[{id:'M',label:'Masculino'},{id:'F',label:'Femenino'},];return gender;};var parseHour=function(hour){if(hour<=9){hour="0"+hour;}
return hour;};var replaceText=function(element,textSearch,textReplace){textSearch.forEach(function(value,index){element=element.replace(value,textReplace);});return element;};var addHourByMin=function(hour){var hoursFinal="";var hoursArray=hour.split(":");var hourMin=parseInt(replaceText(hoursArray[1],["AM","PM"],"").trim());hour=replaceText(hour,["AM","PM",""],"");hour=hour.substring(0,2);hour=hour.replace(":","");if(hourMin<45){hourMin=hourMin+15;}else{hour=parseInt(hour)+1;hourMin="00";}
hour=parseHour(hour);hoursFinal=hour+":"+hourMin+":00";return hoursFinal;};var convertFechaYYMMDD=function(fecha,idioma,options){var newFecha=new Date(fecha).toLocaleDateString(idioma,options);var arrayFecha=newFecha.split("/");newFecha=arrayFecha[2]+"-"+arrayFecha[1]+"-"+arrayFecha[0];return newFecha;};var convertTextToDate=function(language,options,date=null){if(date!=null){return new Date(date).toLocaleDateString(language,options);}else{return new Date().toLocaleDateString(language,options);}};var defineTimeSytem=function(time){var splitTime=time.split(":");var systemTime=splitTime[0]<12?"AM":"PM";var newTime=splitTime[0]+":"+splitTime[1]+" "+systemTime;return newTime;};var getAsUriParameters=function(data){var url='';for(var prop in data){url+=encodeURIComponent(prop)+'='+
encodeURIComponent(data[prop])+'&';}
return url.substring(0,url.length-1)}
var getDayText=function(index,option){var days=getDaysWeek();var dayText=days[index].label;if(option=="short"){dayText=dayText.substr(0,1);}
return dayText;};var messageAlert=function(title,text,type,time=2000){swal({title:title,text:text,type:type,timer:time,showConfirmButton:false});};var messageErrorApi=function(data,title,type){var errorJson=JSON.stringify(data);if(errorJson.indexOf("error")>0){messageAlert(title,data.error.user_msg,type);}else{messageAlert(title,data,type);}};var historyBack=function(){window.history.back();};var cleanString=function(cadena){var specialChars="!@#$^&%*()'+=-[]\/{}|:<>?,";for(var i=0;i<specialChars.length;i++){cadena=cadena.replace(new RegExp("\\"+specialChars[i],'gi'),'');}
cadena=cadena.toLowerCase();cadena.replace(/\s/g,"");cadena=cadena.replace(/[áàäâå]/gi,"a");cadena=cadena.replace(/[éèëê]/gi,'e');cadena=cadena.replace(/[íìïî]/gi,'i');cadena=cadena.replace(/[óòöô]/gi,'o');cadena=cadena.replace(/[úùüû]/gi,'u');cadena=cadena.replace(/[ýÿ]/gi,'y');cadena=cadena.replace(/ñ/gi,'n');return cadena;};var convertFechaYYMMDD=function(fecha,idioma,options){var newFecha=new Date(fecha).toLocaleDateString(idioma,options);var arrayFecha=newFecha.split("/");newFecha=arrayFecha[2]+"-"+arrayFecha[1]+"-"+arrayFecha[0];return newFecha;};angular.module('form.directive',[]).directive('nextOnEnter',function(){function nextForm(scope,element,attr,ctrl){element.bind('keydown',function(e){var code=e.keyCode||e.which;if(code===13){e.preventDefault();var pageElems=document.querySelectorAll('input, select, textarea'),elem=e.srcElement
