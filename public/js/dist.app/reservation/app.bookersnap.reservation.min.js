...
$scope.cancel=function(){$modalInstance.dismiss('cancel');};}).controller('ZoneInstanceCtrl',function($scope,$uibModal,$modalInstance,$filter,content){$scope.itemTables=[];$scope.listZones=content;console.log($scope.listZones);$scope.activarTableOptions=function(index,data){var numero=$scope.itemTables.length;if(numero>0){var index=$scope.itemTables.indexOf(data);if(index>-1){$scope.itemTables.splice(index,1);}else{$scope.itemTables.push(data);}}else{$scope.itemTables.push(data);}
console.log('Seleccionados: '+angular.toJson($scope.itemTables,true));};$scope.addPrecio=function(){modalInstancesPrices()};function modalInstancesPrices(){var modalInstance=$uibModal.open({templateUrl:'myModalContentPrice.html',controller:'PriceInstanceCtrl',size:'sm',resolve:{content:function(){return $scope.itemTables;}}});}
$scope.cancel=function(){$modalInstance.dismiss('cancel');};}).controller('PriceInstanceCtrl',function($scope,$modalInstance,$filter,content){$scope.itemTables=content;$scope.cancel=function(){$modalInstance.dismiss('cancel');};});angular.module('promotion.directive',[]).directive('ngDraggable',function($document,$window){function makeDraggable(scope,element,attr){var startX=0;var startY=0;var x=Math.floor((Math.random()*300)+40);var y=Math.floor((Math.random()*200)+40);element.css({position:'absolute',cursor:'pointer',top:y+'px',left:x+'px'});element.on('mousedown',function(event){event.preventDefault();startX=event.pageX-x;startY=event.pageY-y;$document.on('mousemove',mousemove);$document.on('mouseup',mouseup);});function mousemove(event){y=event.pageY-startY;x=event.pageX-startX;element.css({top:y+'px',left:x+'px'});}
