...
$scope.saveFlyer=function(){if($scope.fileimg){angular.forEach($scope.textFlyer,function(data,index){data.coodinates={x:angular.element('.text-flyer').eq(index).css("left"),y:angular.element('.text-flyer').eq(index).css("top")}})
$scope.principal={"microsite_id":1,"event_id":1,"token":"abc123456","status":$scope.flyer.stateSelected.value,"image":$scope.coleccion.fileimg.name,"label":$scope.textFlyer};console.log("General  "+angular.toJson($scope.textFlyer,true));}else{messageAlert("Flyer","Debe seleccionar una imagen para el flyer","warning");};}
