...
var idMicrositio=obtenerIdMicrositio();angular.module('reservation.app',['promotion.app']).constant("IdMicroSitio",idMicrositio).constant("ApiUrlReservation","http://web.aplication.bookersnap/v1/en/admin/ms/"+idMicrositio+"/reservation").constant("ApiUrlGeneral","http://192.168.0.107/api.promociones/public/v1/es");angular.module('promotion.app',['promotion.controller','promotion.service','promotion.directive','promotion.filter']).config(function($stateProvider,$urlRouterProvider,$httpProvider){$stateProvider.state('promotion',{url:'/promotion',templateUrl:'/js/app/reservation/promotion/view/index.html',controller:'PromotionCtrl',}).state('promotion-list',{url:'/promotion/list',templateUrl:'/js/app/reservation/promotion/view/promotion-list.html',controller:'PromotionListCtrl',}).state('add',{url:'/promotion/add',views:{'':{templateUrl:'/js/app/reservation/promotion/view/add.html'},'promotion@add':{templateUrl:'/js/app/reservation/promotion/view/promotion-add.html',controller:'PromotionAddCtrl',},'flyer@add':{templateUrl:'/js/app/reservation/promotion/view/flyer-add.html',controller:'FlyerAddCtrl',}}});});angular.module('promotion.controller',['infinite-scroll','ui.sortable','sortable','LocalStorageModule','ngFileUpload','ngImgCrop','textAngular','ngEmoticons','farbtastic','localytics.directives']).controller('PromotionCtrl',function($scope){$scope.titulo="Promociones";}).controller('PromotionListCtrl',function($scope,$http){$scope.images=[1,2,3,4,5,6,7,8];$scope.loadMore=function(){var last=$scope.images[$scope.images.length-1];for(var i=1;i<=8;i++){$scope.images.push(last+i);}};$scope.init=function(){$scope.promociones=[];$http.get("http://192.168.0.107/api.promociones/public/v1/es/microsites/1/promotions").success(function(response){if(response.success==true){$scope.promociones=response.data;}}).error(function(){});$scope.show="All";$scope.currentShow=0;};$scope.deleteTodo=function(item){var r=confirm("¿Desea eliminar la promoción?");if(r==true){$http({method:'DELETE',params:{token:"sdsdf5sdf56sd6f5"},url:'http://192.168.0.107/api.promociones/public/v1/es/microsites/1/promotions/'+item.id,}).success(function(response){if(response["success"]==true){var index=$scope.promociones.indexOf(item);$scope.promociones.splice(index,1);}else if(response["success"]==false){messageAlert("Error",response["msg"],"warning");}}).error(function(data){alert(data.jsonError);});}};$scope.todoSortable={containment:"parent",cursor:"move",tolerance:"pointer"};$scope.init();var items=[];$scope.$watch("promociones",function(newVal,oldVal){if(newVal!==null&&newVal.length!=0&&oldVal.length!=0){if(newVal!=oldVal&&oldVal!=null){var newurl="";for(i=0;i<newVal.length;i++){var myData=new Array("items["+i+"][promotion_id]="+newVal[i].id+"","items["+i+"][item]="+newVal[i].item+"");var url=myData.join('&');if(i==0){newurl=newurl.concat(url);}else{newurl=newurl.concat("&"+url);}}
$http({method:'PATCH',url:'http://192.168.0.107/api.promociones/public/v1/es/microsites/1/promotions/order?'+newurl,}).then(function successCallback(response){if(response["success"]==false){messageAlert("Error",response["msg"],"warning");}},function errorCallback(response){console.log(response.statusText)});}}},true);$scope.estadoPromocion=function(index){var status=0;var id=$scope.promociones[index].id;if($scope.promociones[index].status==true){status=1;}else if($scope.promociones[index].status==false){status=0;}
$http({method:'PATCH',url:'http://192.168.0.107/api.promociones/public/v1/es/microsites/1/promotions/'+id+'?status='+status,}).then(function successCallback(response){},function errorCallback(response){});}
var move=function(origin,destination){var temp=$scope.promociones[destination];$scope.promociones[destination]=$scope.promociones[origin];$scope.promociones[origin]=temp;};$scope.moveUp=function(index){move(index,index-1);};$scope.moveDown=function(index){move(index,index+1);};}).controller('PromotionAddCtrl',function($scope,Upload,$timeout,$uibModal,PromotionFactory,TurnosPromotionFactory){$scope.titulo="Nueva promoción";PromotionFactory.getTypes().success(function(data){var vTypes=[];angular.forEach(data['data'],function(types){vTypes.push(types);});$scope.promotion.tipos=vTypes;$scope.promotion.tipoSelected=$scope.promotion.tipos[0];});$scope.promotion={estados:[{name:'Activo',value:1},{name:'Inactivo',value:0}],estadoSelected:{value:1},descripcion:" ",zonas:[{id:1,title:'Zona01'},{id:2,title:'Zona02'},{id:3,title:'Zona03'},{id:4,title:'Zona04'}],zonaSelected:[2,3],caduca:false,publica:true,myImage:undefined};$scope.cropped={cropWidth:100,cropHeight:100,cropTop:0,cropLeft:10}
