...
$scope.typeDrag="";$scope.indexTable=null;$scope.selectedTable=false;this.alertaPrueba=function(){};$scope.onDragComplete=function(data,evt,type){$scope.typeDrag=type;selectTableTypeDrag(data,type);};$scope.onDropComplete=function(data,evt){data.top=evt.y;data.left=evt.x;console.log("onDropComplete "+evt.x+" - "+evt.y);selectTableTypeDrop(data);$scope.typeDrag="";};$scope.onLienzo=function(){if($scope.indexTable!=null&&$scope.selectedTable==false){console.log("onLienzo");$scope.activarTablesItems();}
...
updateHeaderZone();};$scope.rotateShapeTable=function(){console.log("rotateShapeTable ",$scope.itemTables[$scope.indexTable].rotate);if($scope.itemTables[$scope.indexTable].rotate=="0"){$scope.itemTables[$scope.indexTable].rotate="45";}else{$scope.itemTables[$scope.indexTable].rotate="0";}};$scope.activarTableOptions=function(index,vthis){$scope.selectedTable=true;getDataTableSelected(index);$scope.$apply(function(){$scope.boxTables.item=true;$scope.boxTables.items=false;});};$scope.doneTableSelected=function(){$scope.activarTablesItems();};var listCovers=function(option){var coverList="";if(option=="min"){coverList=$scope.coversList.dataMin;}else{coverList=$scope.coversList.dataMax;}
