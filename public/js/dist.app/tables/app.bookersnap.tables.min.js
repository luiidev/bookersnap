...
var idMicrositio=obtenerIdMicrositio();angular.module('tables.app',['turn.app','zone.app']).constant("IdMicroSitio",idMicrositio).constant("ApiUrl",'http://web.aplication.bookersnap/v1/en/admin/ms/'+idMicrositio+'/mesas');angular.module('turn.app',['turn.controller','turn.service','turn.directive']).config(function($stateProvider,$urlRouterProvider){$stateProvider.state('turn',{url:'/config/turn',templateUrl:'/js/app/tables/turn/view/index.html'}).state('turn.active',{url:'/turn-active',templateUrl:'/js/app/tables/turn/view/turn-active.html'}).state('turn.inactive',{url:'/turn-inactive',templateUrl:'/js/app/tables/turn/view/turn-inactive.html'}).state('turn-create',{url:'/config/turn/new',templateUrl:'/js/app/tables/turn/view/turn-create.html'}).state('turn-edit',{url:'/config/turn/edit',templateUrl:'/js/app/tables/turn/view/turn-edit.html'})});angular.module('turn.controller',[]).controller('TurnCtrl',function($scope){}).controller('TurnCreateCtrl',function($scope){});angular.module('turn.directive',[]);angular.module('turn.service',[]);angular.module('zone.app',['zone.controller','zone.service','zone.directive']).config(function($stateProvider,$urlRouterProvider){$stateProvider.state('zone',{url:'/config/zone',templateUrl:'/js/app/tables/zone/view/index.html'}).state('zone.active',{url:'/zone-active',templateUrl:'/js/app/tables/zone/view/zone-active.html'}).state('zone.inactive',{url:'/zone-inactive',templateUrl:'/js/app/tables/zone/view/zone-inactive.html'}).state('zone-create',{url:'/config/zone/new',templateUrl:'/js/app/tables/zone/view/zone-create.html'}).state('zone-edit',{url:'/config/zone/:id',templateUrl:'/js/app/tables/zone/view/zone-edit.html'})});angular.module('zone.controller',['ngDraggable']).controller('ZoneCtrl',function($scope,ZoneFactory){$scope.zonesActive={};$scope.zonesInactive={};$scope.getZones=function(){ZoneFactory.getZones().success(function(data){var vZonesActive=[];var vZonesInactive=[];angular.forEach(data,function(zones){var zonesTables=getTablesCount(zones);if(zones.status=="0"||zones.status=="2"){vZonesInactive.push(zonesTables);}else{vZonesActive.push(zonesTables);}});$scope.zonesActive=vZonesActive;$scope.zonesInactive=vZonesInactive;});};var getTablesCount=function(zones){var vTables=0;angular.forEach(zones.tables,function(tables){vTables+=1;});zones.tables_count=vTables;return zones;};$scope.getZones();}).controller('ZoneCreateCtrl',function($scope,$stateParams,ZoneFactory,ZoneLienzoFactory,TableFactory,$uibModal,IdMicroSitio){$scope.sizeTableList={data:[{id:"1",label:"small"},{id:"2",label:"medium"},{id:"3",label:"large"}],selectedOption:{id:"1",label:"small"}}
...
updateHeaderZone();};$scope.rotateShapeTable=function(){console.log("rotateShapeTable ",$scope.itemTables[$scope.indexTable].rotate);if($scope.itemTables[$scope.indexTable].rotate=="0"){$scope.itemTables[$scope.indexTable].rotate="45";}else{$scope.itemTables[$scope.indexTable].rotate="0";}};$scope.activarTableOptions=function(index,vthis){getDataTableSelected(index);$scope.$apply(function(){$scope.boxTables.item=true;$scope.boxTables.items=false;});};$scope.doneTableSelected=function(){$scope.activarTablesItems();};var listCovers=function(option){var coverList="";if(option=="min"){coverList=$scope.coversList.dataMin;}else{coverList=$scope.coversList.dataMax;}
...
dataZone.tables.push(tableItem);});if(option=="create"){ZoneFactory.createZone(dataZone).success(function(response){console.log("succes createZone "+JSON.stringify(response));});}else{ZoneFactory.editZone(dataZone).success(function(response){console.log("succes editZone "+JSON.stringify(response));});}
