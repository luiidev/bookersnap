angular.module('promotionList.controller', ['ui.sortable', 'sortable'])
    .controller('PromotionListCtrl', function($rootScope, $scope, $http, $window, $document, ApiUrlReservation) {
        ...
        $scope.filtrar = function() {
            ...
            $http.get(
                    ApiUrlReservation + '/promotions?page=' + $scope.page + '&limit=' + $scope.limit + '&filter[fecha_inicial]=' + $scope.obtenerFecha($rootScope.filtro.fecha_inicio) + '&filter[fecha_final]=' + $scope.obtenerFecha($rootScope.filtro.fecha_fin) + '&filter[name]=' + $rootScope.filtro.texto)
                .then(function(items) {
            ...
            angular.element($window).bind("scroll", function() {
                if (this.window.innerHeight + this.window.window.scrollY >= $document[0].body.offsetHeight) {
                    ...
                    if ($scope.fetching === true) {
                        $http.get(ApiUrlReservation + '/promotions?page=' + $scope.page + '&limit=' + $scope.limit + '&filter[fecha_inicial]=' + $scope.obtenerFecha($rootScope.filtro.fecha_inicio) + '&filter[fecha_final]=' + $scope.obtenerFecha($rootScope.filtro.fecha_fin) + '&filter[name]=' + $rootScope.filtro.texto)
                            .then(function(items) {
                                $scope.fetching = items.data.success;
        ...
        */
        ...
        $scope.fetchMore = function() {
            ...
            if (end) return;
            $scope.page++;
            var url = ApiUrlReservation + '/promotions?page=' + $scope.page + '&limit=' + $scope.limit + '&filter[fecha_inicial]=' + $scope.obtenerFecha($rootScope.filtro.fecha_inicio) + '&filter[fecha_final]=' + $scope.obtenerFecha($rootScope.filtro.fecha_fin) + '&filter[name]=' + $rootScope.filtro.texto;
            console.log(url);
            console.log("fetch more: ", $scope.page);
            ...
            $http.get(url).success(function(items) {
                ...
                if (items.data.length) {
                    for (var i = 0; i < items.data.length; i++) {
                        $scope.promociones.push(items.data[i]);
                    }
